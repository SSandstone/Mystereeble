...
const guesses=document.querySelectorAll('.wordGuessed');
...
- *DONE* - add alphabet between grid and guess div and show the color of them
- *DONE* - make them have to use an acceptable word to guess 
...
// will try to clean up code in future
...
function virtualType(){
    const vKey=document.querySelectorAll('.alphabet');
    let index=0;

    vKey.forEach(key => key.addEventListener('click', function(){
...
function guessInputArea(){
        const guessCell=document.createElement('input');
        guessCell.type='text';
        guessCell.name='wordGuessed'
        guessCell.required='true';
        guessCell.minLength=`${wordLength}`;
...
function submitGuess(){
    ...
    const guesses=document.querySelector('.wordGuessed');
    let guessedWord=guesses.value.toUpperCase();
    guessList=guessedWord.split("");
